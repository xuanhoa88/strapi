...
const getModelsFrom = (source) =>
  _.flatMap(source, (iteratee) => _.values(iteratee.models))

/**
 * Checks that there are no model using reserved names (content type,  attributes)
 */
module.exports = ({ strapi, manager }) => {
  ;[...getModelsFrom(strapi.api), ...getModelsFrom(strapi.plugins)].forEach(
    (model) => {
