...
module.exports = {
  ...
  fetchPage(options) {
    ...
    const { tableName } = targetModel.prototype
    ...
    const metadata =
      !options.limit && !options.offset ? { page, pageSize } : { offset, limit }

    if (options.disableCount) {
      return paginate().then((rows) =>
        Object.assign(rows, { pagination: metadata })
      )
    }
  ...
  morphCandidate(candidates, morphValue) {
    const Target = _.find(
      candidates,
    ...
    if (!Target)
      throw new Error(
        `The target polymorphic type "${morphValue}" is not one of the defined target types`
      )
