...
_.extend(Sync.prototype, {
  ...
  first: Promise.method(function (attributes) {
    ...
    const whereAttributes = _.omitBy(
      attributes,
      (attribute, name) =>
        _.isPlainObject(attribute) || name === model.idAttribute
    )
  ...
  count: Promise.method(function (column) {
    ...
    const { options } = this
    const { relatedData } = this.syncing
    const fks = {}
    ...
    return Promise.bind(this)
      ...
      .then(() => knex.count(`${column || '*'} as count`))
