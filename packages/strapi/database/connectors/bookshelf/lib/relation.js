...
const Relation = RelationBase.extend(
  /** @lends Relation.prototype */ {
    ...
    key(keyName) {
      ...
      switch (keyName) {
        case 'otherKey':
          this[keyName] = `${singularMemo(this.targetTableName)}_${
            this.targetIdAttribute
          }`
          break
        case 'throughForeignKey':
          this[keyName] = `${singularMemo(this.joinTable())}_${
            this.throughIdAttribute
          }`
          break
        case 'foreignKey':
          switch (this.type) {
            ...
            default:
              ...
              this[keyName] = `${singularMemo(this.parentTableName)}_${
    ...
    joinColumns(knex) {
      ...
      knex.columns(
        _.map(columns, (col) => `${joinTable}.${col} as _pivot_${col}`)
      )
    },
    ...
    eagerPair(relationName, related, parentModels, options) {
      ...
      if (this.type === 'morphTo') {
        parentModels = _.filter(
          parentModels,
          (m) => m.get(this.key('morphKey')) === this.key('morphValue')
        )
      }
...
const PivotHelpers = {
   ...
   */
  updatePivot(attributes, options) {
  ...
  _processPlainPivot: Promise.method(function (method, item, options, data) {
    ...
    return this.triggerThen('creating', this, data, options).then(() =>
      builder.insert(data).then(() => {
        collection.add(item)
      })
    )
  }),
   ...
   * @return {Promise}
   */
  _processModelPivot: Promise.method(function (
    method,
    item,
    options,
    data,
    ...
    const { relatedData } = this
    const JoinModel = relatedData.throughTarget
    const joinModel = new JoinModel()
